...
- **资源管理**：AI 网关实例池、AI 业务集群（Cluster）。
- **路由管理**：默认路由规则、AI 高级路由规则（AI Route Rules）。
- **消费者管理**：API Key、Entity（服务实体）。
- **用户管理**：登录用户、Token 管理。
...
│  例如：Cluster 的 BaseConfig、Timeout、InstancePool 等步骤组件 │
├────────────────────────────────────────────────────────────┤
│                      公共能力层（Common）                    │
...
- `components/index.vue`：新建/编辑向导容器，按步骤组合 `BaseConfig`、`Timeout`、`PassiveHealthCheck`、`InstancePool`、`GatewayConfig`、`Review`。
...
- `/`：`BaseView` 作为布局容器，重定向到首页，子路由为各业务模块：
  ...
  - `DefaultRouteRule.list`：默认路由规则。
  - `AdvanceRouteRule.list`：AI 高级路由规则。
...
| 组件 | 职责 |
|------|------|
| `pageTable` | 统一表格：数据、列配置、加载、排序、搜索。 |
| `CustomModal` | 统一弹窗封装。 |
| `Expression` | 路由规则中的条件表达式可视化编辑。 |
...
- 子表单组件（如 `BaseConfig`、`Timeout`、`InstancePool`、`GatewayConfig`）。
- 详情/复核组件（`Review.vue`）。
...
| `tableData` | `Array` | 否 | 原始数据，组件内部完成分页、搜索、排序。 |
...
| `api-define/OpenAPI接口依赖说明.md` | 如何引用 ai-gateway-api 的 OpenAPI。 |
