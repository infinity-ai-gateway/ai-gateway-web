window.PrototypeNav = [
  {
    ...
    i18n: 'ResourceManage',
    text: '资源管理',
    icon: 'ivu-icon ivu-icon-md-appstore',
    children: [
      { id: 'AIGatewayInstancePool.list', i18n: 'AIGatewayInstancePoolManage', page: 'instance-pool-ai.html', text: 'AI网关实例池', icon: 'iconfont icon-instancePool' },
      { id: 'AICluster.list', i18n: 'AIClusterManage', page: 'cluster-list.html', text: 'AI业务集群', icon: 'iconfont icon-jiqun' }
    ]
  },
  {
    id: 'route.admin.list',
  ...
  },
  {
    ...
    children: [
      { id: 'APIKey.list', i18n: 'APIKeyManage', page: 'api-key.html', text: 'API Key 管理', icon: 'ivu-icon ivu-icon-ios-key' },
      { id: 'Entity.list', i18n: 'EntityManage', page: 'entity.html', text: 'Entity管理', icon: 'ivu-icon ivu-icon-ios-settings' }
    ]
  },
  { id: 'user.list', i18n: 'UserManage', page: 'user.html', text: '用户管理', icon: 'iconfont icon-user' }
];
...
function findNavLabel(pageId) {
  var found = window.Prototype && typeof Prototype.t === 'function'
...
function renderMenuItem(item, pageId, basePath) {
  if (item.children) {
    var sub = item.children.map(function (child) {
      var active = child.id === pageId ? ' ivu-menu-item-active ivu-menu-item-selected' : '';
      var href = basePath + 'pages/' + child.page;
      return '<li class="ivu-menu-item' + active + '" name="' + child.id + '">' +
        '<a href="' + href + '" class="menu-link">' +
        '<i class="' + child.icon + '"></i> ' + navLabel(child) + '</a></li>';
    }).join('');
    return '<li class="ivu-menu-submenu ivu-menu-opened">' +
      '<div class="ivu-menu-submenu-title">' +
      '<i class="' + item.icon + '"></i> ' + navLabel(item) +
      '<i class="ivu-icon ivu-icon-ios-arrow-down ivu-menu-submenu-title-icon"></i></div>' +
      '<ul class="ivu-menu">' + sub + '</ul></li>';
  }
  var activeTop = item.id === pageId ? ' ivu-menu-item-active ivu-menu-item-selected' : '';
  return '<li class="ivu-menu-item' + activeTop + '">' +
    '<a href="' + basePath + 'pages/' + item.page + '" class="menu-link">' +
    '<i class="' + item.icon + '"></i> ' + navLabel(item) + '</a></li>';
}
...
window.Layout = {
  render(options) {
    ...
    var productTitle = window.Prototype ? Prototype.t('login.gateway') : 'AI网关';
    ...
    var logoutText = window.Prototype ? Prototype.t('com.cancellation') : '注销';
    var confirmTitle = window.Prototype ? Prototype.t('com.informationTips') : '信息提示';
    var confirmLogout = window.Prototype ? Prototype.t('login.tipConfirmLogout') : '确认注销？';
    ...
    document.body.innerHTML =
      '<div id="product-body" style="height:100%;" data-proto-base="' + basePath + '">' +
        '<div class="app-layout">' +
          '<div class="bfe-sidebar">' +
            '<div class="header"><p class="text">' + productTitle + '</p></div>' +
            '<div class="Menu">' +
          ...
          '<div class="bfe-right-content">' +
              ...
              '<li>' +
                '<div class="ivu-dropdown proto-header-dropdown" data-proto-dropdown="lang">' +
                  '<a href="javascript:void(0)" class="proto-dropdown-trigger header_box header_name">' +
                    '<img class="img proto-header-lang-icon" src="' + basePath + 'assets/static/img/loginBg.png" alt="" />' +
                    '<span class="proto-lang-label">' + langLabel + '</span>' +
                  ...
                  '<div class="ivu-select-dropdown proto-dropdown-menu">' +
                    '<ul class="ivu-dropdown-menu">' +
                      '<li class="ivu-dropdown-item' + (lang === 'zh' ? ' ivu-dropdown-item-selected' : '') + '" data-lang="zh">简体中文</li>' +
                      '<li class="ivu-dropdown-item' + (lang === 'en' ? ' ivu-dropdown-item-selected' : '') + '" data-lang="en">English</li>' +
                    '</ul>' +
                  '</div>' +
                '</div>' +
              '</li>' +
                ...
                '<div class="ivu-dropdown proto-header-dropdown" data-proto-dropdown="user">' +
                  '<a href="javascript:void(0)" class="proto-dropdown-trigger header_name proto-header-user">' +
                    '<span class="proto-header-user-icon" aria-hidden="true">' +
                      ...
                      '</svg>' +
                    '</span>' +
                    '<span class="proto-user-name">' + (user.name || 'admin') + '</span>' +
                    '<span class="proto-dropdown-arrow" aria-hidden="true">▾</span>' +
                  '</a>' +
                  '<div class="ivu-select-dropdown proto-dropdown-menu">' +
                    '<ul class="ivu-dropdown-menu">' +
                      '<li class="ivu-dropdown-item" data-action="logout">' + logoutText + '</li>' +
                    '</ul>' +
                  '</div>' +
                '</div>' +
              '</li>' +
            ...
            '<div class="bfe-content">' +
              '<div class="bfe-breadcrumb ivu-breadcrumb">' +
                '<span><span class="ivu-breadcrumb-item-link">' + breadcrumb + '</span></span>' +
              '</div>' +
          ...
          '<div class="ivu-modal-content">' +
            '<div class="ivu-modal-header proto-confirm-header-wrap">' +
              '<div class="proto-confirm-header-inner">' +
                ...
                '<span class="proto-confirm-header-title" id="proto-confirm-title">' + confirmTitle + '</span>' +
              '</div>' +
            '</div>' +
            '<div class="ivu-modal-body">' +
              '<p id="proto-confirm-content" class="proto-confirm-content">' + confirmLogout + '</p>' +
            '</div>' +
            '<div class="ivu-modal-footer proto-confirm-footer">' +
              '<button type="button" class="ivu-btn" id="proto-confirm-cancel">' + confirmCancel + '</button>' +
              '<button type="button" class="ivu-btn ivu-btn-primary" id="proto-confirm-ok">' + confirmOk + '</button>' +
            '</div>' +
          '</div>' +
        '</div>' +
      '</div>';
