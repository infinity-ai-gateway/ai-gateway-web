...
- **配额信息**：配额类型（无限/有限）、配额总量、已使用、剩余量、使用进度条、重置周期。有限配额时可点击「重置配额」按钮重置。
...
| 字段 | 必填 | 默认值 | 格式要求 | 说明 |
| --- | --- | --- | --- | --- |
| 名称 | 是 | 空 | 1-64 字符；不能包含前导或尾随空格；不能包含控制字符 | 全局唯一，创建后不可修改 |
...
| 配额总量 | 是 | `100000000` | 非负整数，周期内可用 Token 总量 |
| 配额单位 | — | `total_token` | 固定为 total_token |
...
点击操作列的「管理路由规则」按钮，跳转到路由规则页面，自动筛选出属主为该组织的路由规则。详见 [08 章](08-route.md)。
