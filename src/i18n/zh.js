...
export default {
  ...
  gatewayConfig: {
    ...
    matchPrefix: '匹配前缀',
    matchPrefixTip: '需要匹配的 provider/model 前缀，例如 openrouter/，必须以 / 结尾',
  ...
  provider: {
    ...
    modelList: '模型列表',
    syncModels: '同步模型',
    modelsHint: '可手动增删，点提交后保存；同步模型会立即覆盖已保存目录',
    modelsHintCreate: '可手动添加。保存服务商后，才能从上游同步模型',
    modelsListTip: '模型列表是该服务商已保存的目录，集群只能从这里选择转发模型。可手动增删，点提交后写入。「同步模型」会立即用上游列表覆盖已保存目录，使用的是已保存的实例、协议、模型列表接口和 Keys 第一项；未保存的修改不会生效。',
    manualModelPlaceholder: '手动添加模型名',
    addModel: '添加',
    instancePool: '服务商实例池',
    ...
    discoverNeedSave: '请先保存服务商后再同步',
    discoverNeedKey: '已保存的 Keys 为空，无法同步。请先配置并提交至少一项 Key，同步会使用第一项作为认证凭证',
    discoverNeedInstance: '已保存的实例池为空，无法同步。请先配置并提交实例',
    discoverNeedProtocol: '已保存的模型协议为空，无法同步。请先配置并提交模型协议',
    discoverNeedSubmit: '同步使用的是已保存配置。实例、协议、模型列表接口或 Keys 有未保存修改，请先提交',
    syncModelsConfirm: '将用上游返回的模型列表覆盖当前已保存的模型，立即生效。是否继续？',
    syncModelsSucc: '已同步并保存 {count} 个模型',
