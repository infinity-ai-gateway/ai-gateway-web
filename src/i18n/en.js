...
export default {
  ...
  provider: {
    ...
    modelsHint: 'Add or remove manually and save on submit. Sync Models immediately overwrites the saved catalog',
    modelsHintCreate: 'You can add models manually. Sync from upstream is available after the provider is saved',
    modelsListTip: 'The model list is the saved catalog for this provider; clusters can only forward models from it. Manual changes are written on submit. Sync Models immediately overwrites the saved catalog using the saved instances, protocol, model list endpoint, and the first Key. Unsaved edits are not used.',
    manualModelPlaceholder: 'Manually add a model name',
    addModel: 'Add',
    ...
    discoverNeedSave: 'Save the provider before syncing models',
    discoverNeedKey: 'Saved Keys is empty, so sync is unavailable. Add and submit at least one Key; sync uses the first Key as the auth credential',
    discoverNeedInstance: 'Saved instance pool is empty, so sync is unavailable. Add and submit instances first',
    discoverNeedProtocol: 'Saved model protocols is empty, so sync is unavailable. Add and submit a protocol first',
    discoverNeedSubmit: 'Sync uses the saved configuration. Submit unsaved changes to instances, protocol, the model list endpoint, or Keys first',
    syncModelsConfirm: 'This will overwrite the saved model list with the upstream result immediately. Continue?',
    syncModelsSucc: 'Synced and saved {count} models',
